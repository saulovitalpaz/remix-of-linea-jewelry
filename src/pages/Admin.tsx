...
export default function Admin() {
  ...
  if (!user) return <main id="main-content" className="flex min-h-screen items-center justify-center bg-muted/30 p-4 sm:p-6"><section className="admin-panel w-full max-w-md">
    <div className="mb-6 text-center"><img src="/Logo 1.png" alt="Chique Detalhes — painel da equipe" width={220} height={120} className="mx-auto h-24 w-auto object-contain sm:h-28" /><p className="mt-2 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">Painel da equipe</p></div>
  ...
  return <div className="min-h-screen bg-muted/30"><header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur"><div className="store-container flex min-h-16 items-center justify-between gap-4 py-3"><div className="min-w-0"><p className="truncate text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">Painel da equipe</p><p className="truncate text-sm font-semibold">{user.name}</p></div><button className="button-secondary shrink-0 px-3 sm:px-4" onClick={logout}><LogOut size={18} aria-hidden="true" /><span className="hidden sm:inline">Sair</span></button></div></header><main id="main-content" className="store-container py-6 sm:py-10">
    <div className="mb-6 flex flex-wrap items-start justify-between gap-4 sm:mb-8"><div><p className="mb-2 text-sm text-muted-foreground">{ROLE_LABELS[user.role]}</p><h1 className="text-2xl sm:text-3xl">Olá, {user.name}</h1><p className="mt-2 text-sm text-muted-foreground sm:text-base">Acompanhe os produtos e as vendas da loja.</p></div></div>
